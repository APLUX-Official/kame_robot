...
void MiniKame::init() {
  ...
  board_pins[0] = D4; // Servo S0
  board_pins[1] = D5; // Servo S1
  board_pins[2] = D6; // Servo S2
  board_pins[3] = D7; // Servo S3
  board_pins[4] = D8; // Servo S4
  board_pins[5] = D9; // Servo S5
  board_pins[6] = D10; // Servo S6
  board_pins[7] = D11; // Servo S7

  // Trim values for zero position calibration.
//  trim[0] = 10;    //前左大腿  加顺时针转动
//  trim[1] = 6;   //前右大腿   加顺时针转动
//  trim[2] = -5;   //前左小腿
//  trim[3] = -15;     //前右小腿
//  trim[4] = -2;    //后左大腿  加顺时针转动
//  trim[5] = -8;   //后右大腿   加顺时针转动
//  trim[6] = 18;     //后左小腿
//  trim[7] = -13;    //后右小腿
...
void MiniKame::hello() {
  float sentado[] = {90 + 15, 90 - 15, 90 - 10, 90 + 60, 90 + 10, 90 - 10, 90 + 10, 90 - 10};
  ...
  int offset[] = {90 + 15, 40, 90 - 30, 90, 90 + 20, 90 - 20, 90 + 10, 90 - 10};
  int phase[] = {0, 0, 0, 90, 0, 0, 0, 0};

  execute(4, period, amplitude, offset, phase);

  float goingUp[] = {90, 70, 90, 90, 90 - 20, 90 + 20, 90 + 10, 90 - 10};
...
void MiniKame::jump() {
  float sentado[] = {90 + 15, 90 - 15, 90 - 60, 90 + 60, 90 + 15, 90 - 15, 90 + 10, 90 - 10};
  float ap = 20.0;
  float hi = 35.0;
  float salto[] = {90 + ap, 90 - ap, 90 - hi, 90 + hi, 90 - ap * 3, 90 + ap * 3, 90 + hi, 90 - hi};
  moveServos(0, sentado);
  delay(600);
  moveServos(0, salto);
  delay(100);
  home();
